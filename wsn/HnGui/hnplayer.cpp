...
HNPlayer::HNPlayer(QObject *parent) :
    QObject(parent)
{
    app = new QProcess(this);
}

void HNPlayer::play(QString filename, int wid, int width, int height)
{
...
void HNPlayer::seekPos(double second)
{
    char buf[256]= {0};
    sprintf(buf, "seek %lf\n", second);
    app->write(buf);
...
void HNPlayer::setRect(int x, int y, int width, int height)
{
    char buf[256] = {0};
