...
void HNInput::changePosition()
{
    ...
    if (currentPosition == "center") {
        ...
        QPoint topleft(0,0);
#else
        QPoint topleft(QApplication::desktop()->screenGeometry().width()-HN_SCRN_WIDTH,
                       QApplication::desktop()->availableGeometry().top());
#endif
        if(ih > 480 && ih2 > 480)
            //up
            this->setGeometry(topleft.x() + ( HN_SCRN_WIDTH - frmWidth) / 2, 0, frmWidth, frmRealHeight);
        else
            //down
            this->setGeometry(topleft.x() + ( HN_SCRN_WIDTH - frmWidth) / 2,
                              topleft.y() + ( HN_SCRN_HEIGHT - frmRealHeight),
