...
void frmInput::changePosition()
{
    ...
    if (currentPosition == "center") {
        ...
        QPoint topleft(0,0);
#else
        QPoint topleft(QApplication::desktop()->screenGeometry().width()-WINDOW_WIDTH,
                       QApplication::desktop()->availableGeometry().top());
#endif
        if(ih > 480 && ih2 > 480)
            //up
            this->setGeometry(topleft.x() + ( WINDOW_WIDTH - frmWidth) / 2, 0, frmWidth, frmRealHeight);
        else
            //down
            this->setGeometry(topleft.x() + ( WINDOW_WIDTH - frmWidth) / 2,
                              topleft.y() + ( WINDOW_HEIGHT - frmRealHeight),
